  ...
  template: `
    ...
    <div class="mb-6">
      <div class="flex items-center gap-3 mb-1">
        <span class="module-badge module-badge--blue">7.</span>
        <h1 class="text-2xl font-extrabold text-[#071938]">Inventario</h1>
      </div>
    </div>
        ...
        <table class="fm-table">
          ...
          <tbody>
            @for (m of paginatedMovements(); track m.id) {
              <tr>
                <td class="!pl-6">
                  <span class="font-mono text-sm font-semibold text-[#071938]">{{ m.movementNumber }}</span>
                </td>
                <td>
                  <span class="text-gray-600 text-sm">{{ m.movementDate }}</span>
                </td>
                <td>
                  <span
                    class="status-badge"
                    [class]="m.movementType === 'ENTRADA' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-600 border-red-200'"
                  >
                    {{ m.movementType }}
                  </span>
                </td>
                <td>
                  <span class="font-semibold text-[#071938]">{{ m.productDescription }}</span>
                </td>
                <td>
                  <span class="font-semibold text-gray-800 text-sm">{{ m.quantity | number }}</span>
                </td>
              ...
              <tr>
                <td colspan="6" class="py-16 text-center">
                  <div class="flex flex-col items-center gap-2">
                    ...
                    <p class="text-sm font-semibold text-[#071938]">No se encontraron movimientos</p>
...
export class InventoryListComponent {
  ...
  filteredList = computed(() => this.inventoryService.filteredMovements());
